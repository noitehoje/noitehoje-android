...
public class AnalyticsUtils {
    private static final String TAG = "NoiteHojeAnalyticsUtils";
    ...
    private static final String UACODE = "UA-22061370-2";
    ...
    public static AnalyticsUtils getInstance(Context context) {
        ...
        if (sInstance == null) {
            if (context == null || !Consts.RELEASE) {
                return sEmptyAnalyticsUtils;
            }
            ...
            protected Void doInBackground(Void... voids) {
                try {
                    mTracker.trackEvent(category, action, label, value);
		   			if (Consts.DEBUG_API)
        	            Log.d(TAG, "NoiteHoje Analytics trackEvent: "
            	                + category + " / " + action + " / " + label + " / " + value);
                } catch (Exception e) {
                    // We don't want to crash if there's an Analytics library exception.
		   			if (Consts.DEBUG_API)
        	            Log.w(TAG, "NoiteHoje Analytics trackEvent error: "
            ...
            protected Void doInBackground(Void... voids) {
                try {
                    mTracker.trackPageView(path);
		   			if (Consts.DEBUG_API)
        	            Log.d(TAG, "NoiteHoje Analytics trackPageView: " + path);
                } catch (Exception e) {
                    // We don't want to crash if there's an Analytics library exception.
		   			if (Consts.DEBUG_API)
        	            Log.w(TAG, "NoiteHoje Analytics trackPageView error: " + path, e);
                }
                return null;
            }
        }.execute();
